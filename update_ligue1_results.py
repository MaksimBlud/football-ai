...
LEAGUE="LIGUE_1";SEASON="2026-2027";TIMEZONE=ZoneInfo("Europe/Paris")
...
def sync_results(write=False):
    provider=fetch_current_finished_results(LIGUE1_RUNTIME_CONFIG);frame=provider["frame"]
    print("LIGUE 1 FINISHED RESULTS SYNC — PUBLIC FOOTBALL-DATA CSV");print("source:",provider["source_url"]);print("source rows:",provider["source_rows"]);print("finished rows:",len(frame));print("paid provider requests:",provider["paid_provider_requests"])
    ...
    m=persistence.persist_results(supabase,frame,LIGUE1_RUNTIME_CONFIG);out={"inserted":int(m["inserted"]),"unchanged":int(m["unchanged"]),"conflicts":int(m["conflicts"]),"finished_rows":len(frame),"paid_provider_requests":0};print("persistence:",out);print("production model used:",False);print("Structural V2 used:",False);return out
def main():
