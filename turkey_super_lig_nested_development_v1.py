"""Execute the frozen Turkey Super Lig nested historical development V1."""
...
from turkey_super_lig_nested_support import (
    ADMITTED_SEASONS,
    CONFIGURED_SEASONS,
...
DEVELOPMENT_OOS_SEASONS = ("2020-2021", "2021-2022", "2023-2024", "2024-2025", "2025-2026")
OUTER_TEST_SEASONS = ("2023-2024", "2024-2025", "2025-2026")
...
REQUIRED_OUTER_WINS = 2
OUTPUT = Path("experiments/turkey_super_lig_nested_development_v1/latest.json")
...
def arrays(predictions: pd.DataFrame) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    ai = simplex(predictions[["ai_home_probability", "ai_draw_probability", "ai_away_probability"]].to_numpy(float))
    market = simplex(predictions[["market_home_probability", "market_draw_probability", "market_away_probability"]].to_numpy(float))
    return y, ai, market
...
def select_candidate(predictions: dict[tuple[str, str], pd.DataFrame], seasons: tuple[str, ...]):
    rows = []
    for feature_set in FEATURE_SET_ORDER:
        for model_name in MODEL_ORDER:
            score = score_ai(predictions[(feature_set, model_name)], seasons)
            rows.append((score["logloss"], score["brier"], -score["accuracy"], feature_set, model_name, score))
...
def inner_seasons(outer_season: str) -> tuple[str, ...]:
    ...
    if len(result) < 2:
        raise ValueError("Nested outer fold requires at least two earlier clean OOS seasons")
    return result
...
def score_outer(predictions: pd.DataFrame, season: str, alpha: float):
    ...
    return {
        "test_season": season, "matches": int(len(selected)),
        "hybrid_metrics": hybrid_score, "market_metrics": market_score,
        "wins": wins, "all_three_win": bool(all(wins.values())),
    }, y, market, hybrid


def evaluate_gate(outer_rows: list[dict], market_score: dict, hybrid_score: dict, final_all_three: bool) -> dict:
    metric_counts = {metric: sum(bool(row["wins"][metric]) for row in outer_rows) for metric in ("accuracy", "logloss", "brier")}
    ...
    checks = {
        ...
        "outer_accuracy_wins_at_least_2_of_3": metric_counts["accuracy"] >= REQUIRED_OUTER_WINS,
        "outer_logloss_wins_at_least_2_of_3": metric_counts["logloss"] >= REQUIRED_OUTER_WINS,
        "outer_brier_wins_at_least_2_of_3": metric_counts["brier"] >= REQUIRED_OUTER_WINS,
        "outer_all_three_wins_at_least_2_of_3": all_three_count >= REQUIRED_OUTER_WINS,
        "final_development_recipe_all_three": bool(final_all_three),
    }
    return {**checks, "metric_win_counts": metric_counts, "all_three_win_count": int(all_three_count), "passed": bool(all(checks.values()))}


def run(root: Path, work_dir: Path) -> dict:
    assert_frozen_family()
    before = production_state(root)
    history, excluded_rows = build_history(work_dir)
    features = build_trainable_features(history)
    variants = {
        (feature_set, model_name): build_variant_oos(features, feature_set, model_name)
        for feature_set in FEATURE_SET_ORDER for model_name in MODEL_ORDER
    }
    ...
    for outer_season in OUTER_TEST_SEASONS:
        ...
        fold, y, market, hybrid = score_outer(variants[candidate], outer_season, alpha)
        fold.update({
            "inner_oos_seasons": list(inner), "selected_candidate": f"{candidate[0]}::{candidate[1]}",
            "selected_alpha": alpha, "candidate_selection_metrics": candidate_score,
            "alpha_selection_metrics": alpha_score,
        })
        outer_rows.append(fold); targets.append(y); markets.append(market); hybrids.append(hybrid)

    y_outer, market_outer, hybrid_outer = np.concatenate(targets), np.vstack(markets), np.vstack(hybrids)
    outer_market, outer_hybrid = metrics(y_outer, market_outer), metrics(y_outer, hybrid_outer)

    final_candidate, candidate_score = select_candidate(variants, DEVELOPMENT_OOS_SEASONS)
    final_alpha, alpha_score = select_alpha(variants[final_candidate], DEVELOPMENT_OOS_SEASONS)
    ...
    final_market, final_hybrid = metrics(y_final, market_final), metrics(y_final, hybrid_final)
    final_wins = {
    ...
    gate = evaluate_gate(outer_rows, outer_market, outer_hybrid, all(final_wins.values()))
    after = production_state(root)
    if before != after:
        raise RuntimeError("Production artifact state changed during Turkey research")

    return {
        "protocol": PROTOCOL, "league": LEAGUE,
        "status": "ELIGIBLE_FOR_FUTURE_PROTOCOL_FREEZE" if gate["passed"] else "REJECTED_NO_FREEZE_RECOMMENDATION",
        "configured_historical_seasons": list(CONFIGURED_SEASONS),
        "historical_seasons": list(ADMITTED_SEASONS),
        "structurally_excluded_seasons": [EXCLUDED_SEASON],
        "structurally_excluded_source_rows": excluded_rows,
        "current_forbidden_season": CURRENT_FORBIDDEN_SEASON, "current_source_read": False,
        "development_oos_seasons": list(DEVELOPMENT_OOS_SEASONS), "outer_test_seasons": list(OUTER_TEST_SEASONS),
        "historical_rows": int(len(history)), "trainable_rows": int(len(features)),
        "candidate_count": len(FEATURE_SET_ORDER) * len(MODEL_ORDER), "alpha_grid": list(ALPHAS),
        "outer_folds": outer_rows,
        "outer_aggregate": {
            "hybrid": outer_hybrid, "market": outer_market,
            "metric_win_counts": gate["metric_win_counts"], "all_three_win_count": gate["all_three_win_count"],
        },
        "final_development_recipe": {
            "candidate": f"{final_candidate[0]}::{final_candidate[1]}", "alpha": final_alpha,
            "candidate_selection_metrics": candidate_score, "alpha_selection_metrics": alpha_score,
            "hybrid_metrics": final_hybrid, "market_metrics": final_market, "wins": final_wins,
        },
        "gate": {key: value for key, value in gate.items() if key not in {"metric_win_counts", "all_three_win_count", "passed"}},
        "all_admitted_historical_data_is_development_only": True, "untouched_holdout_claimed": False,
        "artifact_created": False, "model_ready": False, "prospective_ai_ready": False,
...
def main() -> int:
    ...
    parser.add_argument("--work-dir", type=Path, default=Path("artifacts/turkey_super_lig_nested_development_v1"))
