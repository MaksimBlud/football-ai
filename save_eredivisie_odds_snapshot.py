"""Eredivisie h2h odds snapshot collector using The Odds API EU region."""
...
REGION="eu"; LEAGUE="EREDIVISIE"; SPORT_KEY=EREDIVISIE_RUNTIME_CONFIG.identity.odds_sport_key
OUTPUT=Path("data/odds_snapshots/eredivisie_h2h_snapshots.csv")
...
def merge_local_history(old_df,new_df):
    ...
    for frame in (old_df,new_df):
        if not frame.empty and not frame["league"].eq(LEAGUE).all(): raise ValueError("Eredivisie snapshot contains foreign league")
...
def build_db_rows(frame):
    ...
    if not frame.empty and not frame["league"].eq(LEAGUE).all(): raise ValueError("Foreign Eredivisie payload")
...
def main():
    ...
    if frame.empty: raise RuntimeError("The Odds API returned no usable Eredivisie h2h odds")
    history=save_local_history(frame); persisted=save_supabase(frame); bookmaker_rows=capture_h2h_bookmaker_snapshots(result["events"],league=LEAGUE,snapshot_time_utc=now)
    print("EREDIVISIE ODDS SNAPSHOT SAVED"); print("snapshot rows:",len(frame)); print("local history rows:",len(history)); print("Supabase response rows:",persisted); print("bookmaker research rows:",bookmaker_rows); print("quota:",result["quota"]); print("Structural V2 used:",False); print("production model used:",False)
