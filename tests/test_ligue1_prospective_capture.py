...
from ligue1_prospective_capture import (
    LEAGUE,
    PROTOCOL,
    TARGET_N,
    capture_market_only,
...
def _row(**overrides):
    row = {
        "league": LEAGUE,
        "source_event_id": "event-ligue1-001",
...
def test_accepts_ligue1_market_only(tmp_path: Path):
    ...
    assert rows[0]["source_event_id"] == "event-ligue1-001"


def test_rejects_other_leagues():
    for league in ("EPL", "SERIE_A", "LA_LIGA", "BUNDESLIGA"):
...
def test_duplicate_is_idempotent_and_conflicting_rewrite_fails_closed(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="conflicting rewrite"):
        capture_market_only(_row(home_odds=1.75), ledger)
    with pytest.raises(ValueError, match="conflicting rewrite"):
...
def test_committed_ligue1_ledger_is_valid_when_present():
    ledger = Path("experiments/ligue1_market_only_v1.csv")
    if ledger.exists():
        count = validate_ledger(ledger)
        assert 0 < count <= TARGET_N


def test_existing_non_epl_and_bundesliga_ledgers_are_untouched():
    non_epl = Path("experiments/non_epl_market_only_v1.csv")
    bundesliga = Path("experiments/bundesliga_market_only_v1.csv")
    assert non_epl.exists()
    assert bundesliga.exists()
    assert hashlib.sha256(non_epl.read_bytes()).hexdigest() == FROZEN_NON_EPL_LEDGER_SHA256
    assert hashlib.sha256(bundesliga.read_bytes()).hexdigest() == FROZEN_BUNDESLIGA_LEDGER_SHA256


def test_no_production_or_live_provider_dependency():
    import inspect
    import ligue1_prospective_capture as module
