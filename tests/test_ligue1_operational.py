...
import scheduled_ligue1_odds_snapshot as scheduler
import update_ligue1_results as results
import persist_ligue1_market_observations as observations

def test_runtime_contract():
    c=LIGUE1_RUNTIME_CONFIG
    assert c.identity.identifier=="LIGUE_1"
    assert c.identity.timezone=="Europe/Paris"
    assert c.identity.odds_sport_key=="soccer_france_ligue_one"
    assert c.historical_source.competition_code=="F1"
...
def test_finished_result_uses_paris_date():
    event={"completed":True,"commence_time":"2026-08-29T22:30:00Z","home_team":"A","away_team":"B","scores":[{"name":"A","score":"1"},{"name":"B","score":"1"}]}
    row=results.build_finished_row(event)
    assert row["match_date"]=="2026-08-30"
    assert row["result"]=="D"

def test_market_observation_stays_market_only():
    frame=pd.DataFrame([{"league":"LIGUE_1","event_id":"e1","home_team":"A","away_team":"B","commence_time_utc":"2026-09-01T18:00:00Z","snapshot_time_utc":"2026-09-01T12:00:00Z","market_home_probability":0.5,"market_draw_probability":0.3,"market_away_probability":0.2,"market_argmax":"H","market_shadow_status":"OK","market_only":True}])
