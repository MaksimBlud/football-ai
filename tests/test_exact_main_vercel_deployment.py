...
def test_health_exposes_exact_deployment_sha(monkeypatch):
    monkeypatch.setenv("DEPLOYMENT_GIT_SHA", "a" * 40)
    monkeypatch.setenv("VERCEL_GIT_COMMIT_SHA", "b" * 40)

    payload = web_app.health()

    assert payload["deployment_git_sha"] == "a" * 40


def test_health_falls_back_to_vercel_git_sha(monkeypatch):
    monkeypatch.delenv("DEPLOYMENT_GIT_SHA", raising=False)
    monkeypatch.setenv("VERCEL_GIT_COMMIT_SHA", "c" * 40)

    assert web_app.health()["deployment_git_sha"] == "c" * 40


def test_health_is_explicit_when_source_identity_is_missing(monkeypatch):
    monkeypatch.delenv("DEPLOYMENT_GIT_SHA", raising=False)
    monkeypatch.delenv("VERCEL_GIT_COMMIT_SHA", raising=False)

    assert web_app.health()["deployment_git_sha"] == "unknown"
