...
from turkey_super_lig_prospective_capture import (
    FIELDNAMES,
    LEAGUE,
    PROTOCOL,
    TARGET_N,
...
def _row(**overrides):
    row = {
        "league": LEAGUE,
        "source_event_id": "event-turkey-001",
        "source_snapshot_time_utc": "2026-09-11T08:00:00+00:00",
        "home_team": "Galatasaray",
        "away_team": "Fenerbahce",
        "commence_time_utc": "2026-09-12T18:00:00+00:00",
...
def test_accepts_turkey_super_lig_market_only(tmp_path: Path):
    ledger = tmp_path / "ledger.csv"
    result = capture_market_only(_row(), ledger)
    assert result.status == "INSERTED"
    assert result.league == LEAGUE == "TURKEY_SUPER_LIG"
    assert result.protocol == PROTOCOL == "TURKEY_SUPER_LIG_MARKET_ONLY_V1"
    ...
    assert rows[0]["source_event_id"] == "event-turkey-001"
...
def test_rejects_other_leagues():
    for league in ("EPL", "SERIE_A", "LA_LIGA", "BUNDESLIGA", "LIGUE_1", "EREDIVISIE", "SUPER_LIG"):
...
def test_rejects_post_kickoff_capture_and_snapshot():
    with pytest.raises(ValueError, match="strictly before kickoff"):
        validate_capture(_row(captured_at_utc="2026-09-12T18:00:00Z"))
    with pytest.raises(ValueError, match="source market snapshot must be strictly before kickoff"):
        validate_capture(_row(source_snapshot_time_utc="2026-09-12T18:00:00Z"))
...
def test_sequence_and_empty_schema_corruption_fail_closed(tmp_path: Path):
    ledger = tmp_path / "ledger.csv"
...
def test_committed_turkey_ledger_is_valid_and_starts_at_zero():
    ledger = Path("experiments/turkey_super_lig_market_only_v1.csv")
...
def test_no_production_live_database_or_provider_dependency():
    import inspect
    import turkey_super_lig_prospective_capture as module
