...
import ligue1_nested_development_v1 as protocol
...
def test_contract_constants_are_frozen():
    assert protocol.LEAGUE == "LIGUE_1"
...
def test_later_historical_season_fails_closed():
    frame = pd.DataFrame(
        {
            "league": ["LIGUE_1"] * (len(protocol.ADMITTED_HISTORICAL_SEASONS) + 1),
...
def test_non_ligue1_data_fails_closed():
    frame = pd.DataFrame(
        {
            "league": ["SERIE_A"] * len(protocol.ADMITTED_HISTORICAL_SEASONS),
            "season": list(protocol.ADMITTED_HISTORICAL_SEASONS),
        }
    )
    with pytest.raises(ValueError, match="pure LIGUE_1"):
        protocol.assert_historical_boundary(frame)
...
def test_curtailed_2019_2020_source_must_match_frozen_structural_shape():
    teams = [f"Team {index:02d}" for index in range(20)]
    all_pairs = [(home, away) for home in teams for away in teams if home != away]
    curtailed = _source_from_pairs(all_pairs[:279])
    protocol.validate_structurally_excluded_source(curtailed, season="2019-2020")
...
def test_source_has_no_live_or_production_write_surface():
    ...
    assert "bundesliga" not in source
