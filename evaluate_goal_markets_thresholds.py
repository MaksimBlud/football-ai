...
# Здесь сохраним каждый прогноз каждого тестового сезона,
# чтобы после walk-forward проверить высокие пороги вероятности.
...
for test_index in range(1, len(seasons)):
    ...
    # Сохраняем каждый out-of-sample прогноз.
    # Важно: это именно прогнозы на сезоне, который модель
    # не использовала для обучения на данном шаге.
    for row_index in range(len(test_df)):
        threshold_predictions.append({
            "season": test_season,
            "over_probability": float(
                over_probabilities[row_index]
            ),
            "actual_over": int(
                actual_over[row_index]
            ),
            "btts_probability": float(
                btts_probabilities[row_index]
            ),
            "actual_btts": int(
                actual_btts[row_index]
            ),
        })
...
print()
print("=" * 72)
print("АНАЛИЗ ВЫСОКИХ ВЕРОЯТНОСТЕЙ")
print("=" * 72)
...
predictions_output = (
    "data/goal_markets_oos_predictions.csv"
)
...
print(
    "Out-of-sample прогнозы сохранены:",
    predictions_output,
)

if predictions_df.empty:
    raise RuntimeError(
        "Нет прогнозов для threshold-анализа."
    )
...
def analyze_threshold(
    ...
    wins = int(
        selected[actual_column].sum()
    )

    hit_rate = (
        selected[actual_column].mean()
    )

    avg_probability = (
        selected[probability_column].mean()
    )

    calibration_error = (
        avg_probability - hit_rate
    )
...
threshold_output = (
    "data/goal_markets_threshold_results.csv"
)
...
for market in [
    ...
    market_df = threshold_df[
        threshold_df["market"] == market
    ]
...
print()
print("Всего out-of-sample матчей:", len(predictions_df))

print(
    "Threshold результаты сохранены:",
    threshold_output,
)
