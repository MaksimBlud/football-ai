"""Frozen Ligue 1 nested historical development protocol V1.

Research only. All admitted completed historical seasons are development evidence.
The structurally curtailed 2019-2020 Ligue 1 season is excluded before any
model result is observed. No prospective outcomes, live database access, paid
odds-provider access, model artifact creation, or production promotion occurs.
"""
...
PROTOCOL_VERSION = "ligue1_nested_development_v1"
LEAGUE = "LIGUE_1"
...
ADMITTED_HISTORICAL_SEASONS = tuple(
    season for season in CONFIGURED_HISTORICAL_SEASONS if season not in STRUCTURALLY_EXCLUDED_SEASONS
)
...
def assert_historical_boundary(frame: pd.DataFrame) -> None:
    if frame.empty:
        raise ValueError("Ligue 1 historical frame is empty")
    ...
    if excluded_present:
        raise ValueError(f"Structurally excluded Ligue 1 seasons are forbidden: {excluded_present}")
...
def _validate_required_source_columns(source: pd.DataFrame, *, season: str) -> None:
    ...
    if missing:
        raise ValueError(f"Ligue 1 {season}: missing source columns {sorted(missing)}")
...
def validate_structurally_excluded_source(source: pd.DataFrame, *, season: str) -> None:
    ...
    if season == "2019-2020":
        if len(teams) != 20:
            raise ValueError(f"Ligue 1 {season}: expected 20 teams in curtailed source")
        if len(source) != 279:
            raise ValueError(f"Ligue 1 {season}: expected 279 played matches in curtailed source")
    pairs = source[["HomeTeam", "AwayTeam"]].astype(str).apply(lambda c: c.str.strip())
    if (pairs["HomeTeam"] == pairs["AwayTeam"]).any():
        raise ValueError(f"Ligue 1 {season}: team cannot play itself")
    if pairs.duplicated().any():
        raise ValueError(f"Ligue 1 {season}: duplicate home/away pairing in curtailed source")
    if not set(source["FTR"].astype(str)).issubset({"H", "D", "A"}):
        raise ValueError(f"Ligue 1 {season}: unexpected result code")
    try:
        validate_complete_double_round_robin(source, season=season)
    except ValueError:
        return
    raise ValueError(f"Ligue 1 {season}: excluded season unexpectedly became structurally complete")
...
def normalize_source(source: pd.DataFrame, *, season: str) -> pd.DataFrame:
    ...
    frame["home_team"] = frame["home_team"].astype(str).str.strip().replace(LIGUE1_RUNTIME_CONFIG.aliases)
    frame["away_team"] = frame["away_team"].astype(str).str.strip().replace(LIGUE1_RUNTIME_CONFIG.aliases)
...
def build_historical_frame(work_dir: Path) -> tuple[pd.DataFrame, dict[str, int]]:
    ...
    for code, season in config.historical_source.season_codes.items():
        if season > MAX_ALLOWED_SEASON:
            raise RuntimeError("Configured Ligue 1 history crosses frozen development boundary")
    ...
    if set(excluded_rows) != set(STRUCTURALLY_EXCLUDED_SEASONS):
        raise RuntimeError("Frozen structurally excluded Ligue 1 season was not verified")
...
def add_warmup_and_elo(history: pd.DataFrame) -> pd.DataFrame:
    ...
    initial = float(LIGUE1_RUNTIME_CONFIG.elo.initial_rating)
    k_factor = float(LIGUE1_RUNTIME_CONFIG.elo.k_factor)
    home_advantage = float(LIGUE1_RUNTIME_CONFIG.elo.home_advantage)
    ...
    features["warmup_ok"] = (
        (features["home_prior_matches"] >= int(LIGUE1_RUNTIME_CONFIG.temporal.min_prior_matches))
        & (features["away_prior_matches"] >= int(LIGUE1_RUNTIME_CONFIG.temporal.min_prior_matches))
    )
    ...
    for feature_set in FEATURE_SET_ORDER:
        ...
        for model_name in MODEL_ORDER:
            ...
            for season in DEVELOPMENT_OOS_SEASONS:
                ...
                if "2019-2020" in set(train["season"].astype(str)):
...
def _arrays(predictions: pd.DataFrame, prefix: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    p = predictions[
        [f"{prefix}_home", f"{prefix}_draw", f"{prefix}_away"]
...
def run(root: Path, work_dir: Path, output_path: Path) -> dict:
    ...
    if before != after:
        raise RuntimeError("Production artifact state changed during Ligue 1 research")
...
def main() -> int:
    ...
    parser.add_argument(
        "--work-dir", type=Path, default=Path("artifacts/ligue1_nested_development_v1")
    )
    parser.add_argument(
        "--output", type=Path, default=Path("experiments/ligue1_nested_development_v1/latest.json")
    )
