"""Frozen Serie A nested historical development protocol V1.

Research only. All completed historical seasons are development evidence.
No prospective outcomes, Supabase access, paid Odds API access, model artifact
creation, or production promotion is performed.
"""
...
from league_model_sweep import FEATURE_SETS, MODEL_VARIANTS, TARGET_MAP, make_model, market_probabilities, metrics
...
PROTOCOL_VERSION = "serie_a_nested_development_v1"
...
def assert_historical_boundary(frame: pd.DataFrame) -> None:
    if frame.empty:
        raise ValueError("Serie A historical frame is empty")
    leagues = set(frame["league"].dropna().astype(str).unique())
    if leagues != {"SERIE_A"}:
        raise ValueError(f"Historical frame must be pure SERIE_A, got {sorted(leagues)}")
...
def normalize_source(source: pd.DataFrame, *, season: str) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Serie A {season}: missing source columns {sorted(missing)}")
    ...
    frame["home_team"] = frame["home_team"].astype(str).str.strip().replace(SERIE_A_RUNTIME_CONFIG.aliases)
    frame["away_team"] = frame["away_team"].astype(str).str.strip().replace(SERIE_A_RUNTIME_CONFIG.aliases)
...
def build_historical_frame(work_dir: Path) -> pd.DataFrame:
    config = SERIE_A_RUNTIME_CONFIG
    frames = []
    for code, season in config.historical_source.season_codes.items():
        if season > MAX_ALLOWED_SEASON:
            raise RuntimeError("Configured Serie A history crosses frozen development boundary")
        path = work_dir / "raw" / f"I1_{code}.csv"
        if not path.exists():
    ...
    combined = combined.sort_values(["match_date", "match_time", "home_team", "away_team"], kind="stable").reset_index(drop=True)
...
def add_warmup_and_elo(history: pd.DataFrame) -> pd.DataFrame:
    ...
    initial = float(SERIE_A_RUNTIME_CONFIG.elo.initial_rating)
    k_factor = float(SERIE_A_RUNTIME_CONFIG.elo.k_factor)
    home_advantage = float(SERIE_A_RUNTIME_CONFIG.elo.home_advantage)
    ...
    features.insert(0, "league", "SERIE_A")
    features["home_prior_matches"] = home_prior
    features["away_prior_matches"] = away_prior
    features["warmup_ok"] = (
        (features["home_prior_matches"] >= int(SERIE_A_RUNTIME_CONFIG.temporal.min_prior_matches))
        & (features["away_prior_matches"] >= int(SERIE_A_RUNTIME_CONFIG.temporal.min_prior_matches))
    )
...
def hybrid_metrics(predictions: pd.DataFrame, alpha: float) -> dict[str, float]:
    ...
    hybrid = alpha * ai + (1.0 - alpha) * market
...
def evaluate_nested(predictions: pd.DataFrame) -> dict:
    ...
    for test_season in OUTER_TEST_SEASONS:
        ...
        test = predictions[(predictions["season"] == test_season) & (predictions["candidate"] == winner)].copy()
        if test.empty:
    ...
    for part in outer_selected_parts:
        ...
        all_hybrid_probabilities.append(alpha * ai + (1.0 - alpha) * market)
    y_outer = np.concatenate(all_targets)
    p_outer_market = np.vstack(all_market_probabilities)
    p_outer_hybrid = np.vstack(all_hybrid_probabilities)
...
def run(root: Path, work_dir: Path, output_path: Path) -> dict:
    ...
    if before != after:
        raise RuntimeError("Production artifact state changed during Serie A research")
...
def main() -> int:
    ...
    parser.add_argument("--work-dir", type=Path, default=Path("artifacts/serie_a_nested_development_v1"))
    parser.add_argument("--output", type=Path, default=Path("experiments/serie_a_nested_development_v1/latest.json"))
