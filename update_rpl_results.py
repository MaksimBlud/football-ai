"""Sync immutable RPL finished results from The Odds API scores endpoint.

Default mode is dry-run. Use --write for durable generic result persistence.
No production model or Structural V2 is used.
"""
...
SEASON = RPL_RUNTIME_CONFIG.finished_results_source.season
SOURCE = "the-odds-api"
SOURCE_COMPETITION = RPL_RUNTIME_CONFIG.identity.odds_sport_key
TIMEZONE = ZoneInfo(RPL_RUNTIME_CONFIG.identity.timezone)
...
def build_finished_row(event: dict) -> dict | None:
    ...
    normalize = RPL_RUNTIME_CONFIG.aliases
    canonical_home = normalize.get(home_team, home_team)
    canonical_away = normalize.get(away_team, away_team)
...
def build_finished_frame(events: list[dict]) -> pd.DataFrame:
    ...
    if frame.duplicated(subset=identity).any():
        raise ValueError("Duplicate RPL finished-result identity from provider")
    return frame


def sync_results(*, write: bool) -> dict:
    provider = get_rpl_scores(days_from=3)
    frame = build_finished_frame(provider["events"])

    print("=" * 88)
    print("RPL FINISHED RESULTS SYNC")
    print("=" * 88)
...
def main() -> None:
    ...
    parser.add_argument(
        "--write",
        action="store_true",
        help="Persist immutable RPL finished results to Supabase.",
    )
