"""Ligue 1 h2h odds snapshot collector using The Odds API EU region."""
...
REGION="eu";LEAGUE="LIGUE_1";SPORT_KEY=LIGUE1_RUNTIME_CONFIG.identity.odds_sport_key;OUTPUT=Path("data/odds_snapshots/ligue1_h2h_snapshots.csv")
...
def merge_local_history(old_df,new_df):
    ...
    for f in (old_df,new_df):
        if not f.empty and not f["league"].eq(LEAGUE).all():raise ValueError("Ligue 1 snapshot contains foreign league")
...
def save_local_history(new_df,output_path=OUTPUT):
    output_path=Path(output_path);output_path.parent.mkdir(parents=True,exist_ok=True);old=pd.read_csv(output_path) if output_path.exists() else pd.DataFrame(columns=DB_COLUMNS);combined=merge_local_history(old,new_df);combined.to_csv(output_path,index=False);return combined
def build_db_rows(frame):
    frame=require_league(frame,legacy_epl=False)
    if not frame.empty and not frame["league"].eq(LEAGUE).all():raise ValueError("Foreign Ligue 1 payload")
...
def main():
    ...
    history=save_local_history(frame);persisted=save_supabase(frame);bookmaker_rows=save_h2h_bookmaker_snapshots(result["events"],league=LEAGUE,snapshot_time_utc=now);print("LIGUE 1 ODDS SNAPSHOT SAVED");print("snapshot rows:",len(frame));print("local history rows:",len(history));print("Supabase response rows:",persisted);print("bookmaker research rows:",bookmaker_rows);print("quota:",result["quota"]);print("Structural V2 used:",False);print("production model used:",False)
