...
TARGET_N = 100
DEFAULT_LEDGER = Path("experiments/primeira_liga_market_only_v1.csv")
...
def _validate_no_ai_fields(row: dict[str, Any]) -> None:
    ...
    if present:
        raise ValueError(
            "AI/Structural fields are forbidden by PRIMEIRA_LIGA_MARKET_ONLY_V1 until a "
            f"separate frozen Primeira Liga AI eligibility contract exists: {present}"
        )
...
def capture_market_only(
    row: dict[str, Any], ledger_path: Path = DEFAULT_LEDGER
) -> CaptureResult:
    normalized = validate_capture(row)
    ...
    same_key = [
        ...
        if item.get("canonical_key") == projection["canonical_key"]
    ]
    if same_key:
        existing = same_key[0]
    ...
    if count >= TARGET_N:
        raise ValueError(
            f"prospective target already reached for {LEAGUE}: {TARGET_N}/{TARGET_N}"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fail-closed Primeira Liga MARKET_ONLY prospective capture"
    )
