"""Adaptive scheduler for Ligue 1 h2h snapshots."""
from datetime import datetime,timezone
from database import supabase
from ligue1_runtime_config import LIGUE1_RUNTIME_CONFIG
import save_ligue1_odds_snapshot as collector
...
def should_collect(rows,now):
    if not rows:return True,"NO_EXISTING_LIGUE1_SNAPSHOTS"
    times=[parse_dt(r.get("snapshot_time_utc")) for r in rows if r.get("snapshot_time_utc")];times=[x for x in times if x]
    if not times:return True,"NO_VALID_LIGUE1_SNAPSHOT_TIMES"
    since=(now-max(times)).total_seconds()/3600;future=[parse_dt(r.get("commence_time_utc")) for r in rows if r.get("commence_time_utc")];future=[x for x in future if x and x>now]
...
def fetch_recent_ligue1_snapshots():
    r=supabase.table(TABLE).select("snapshot_time_utc,commence_time_utc,league").eq("league","LIGUE_1").order("snapshot_time_utc",desc=True).limit(500).execute();return list(r.data or [])
def main():
    now=datetime.now(timezone.utc);rows=fetch_recent_ligue1_snapshots();due,reason=should_collect(rows,now);print("decision:",reason);print("snapshot required:",due)
