...
import pandas as pd
import requests
from historical_corners_early_drift import write_early_drift_reports
...
from season_invariant_corners_v1 import write_invariant_report
...
def main():
    ...
    paths=write_reports(combined,a.output_dir); window,robustness=write_window_reports(combined,a.output_dir); invariant_leagues,invariant_status=write_invariant_report(robustness,a.output_dir); market,incremental=write_market_incremental_reports(combined,a.output_dir); drift_detail,drift_summary=write_early_drift_reports(combined,a.output_dir)
    ...
    print("SEASON INVARIANT CORNERS"); print(invariant_leagues.to_string(index=False)); print(f"PORTABILITY={invariant_status}")
    ...
    print("CORNERS10 EARLY DRIFT"); print(drift_summary.to_string(index=False))
