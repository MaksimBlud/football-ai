...
TARGET_N = 100
DEFAULT_LEDGER = Path("experiments/ligue1_market_only_v1.csv")
...
def _validate_no_ai_fields(row: dict[str, Any]) -> None:
    ...
    if present:
        raise ValueError(
            "AI fields are forbidden by LIGUE_1_MARKET_ONLY_V1 until a separate "
            f"Ligue 1 frozen AI eligibility contract exists: {present}"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fail-closed Ligue 1 MARKET_ONLY prospective capture")
