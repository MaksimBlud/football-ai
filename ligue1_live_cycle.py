...
def run_cycle():
    ...
    if len(durable)!=len(ok):raise RuntimeError("Durable Ligue 1 observation count mismatch")
    guarded=dual_write_guard.execute_dual_write(supabase,persisted,durable,LIGUE1_RUNTIME_CONFIG)
