...
TARGET_N = 100
DEFAULT_LEDGER = Path("experiments/turkey_super_lig_market_only_v1.csv")

FORBIDDEN_FIELDS = {
    "result", "outcome", "settled", "settlement", "home_score", "away_score",
    "score", "ft_home_goals", "ft_away_goals", "winner",
}
AI_FIELDS = {
    "ai_home_probability", "ai_draw_probability", "ai_away_probability",
    "model_probability_home", "model_probability_draw", "model_probability_away",
    "model_version", "model_artifact", "structural_home_probability",
    "structural_draw_probability", "structural_away_probability", "structural_score",
}
FIELDNAMES = [
    "league", "protocol", "target_n", "observation_number", "canonical_key",
    "source_event_id", "source_snapshot_time_utc", "home_team", "away_team",
    "commence_time_utc", "captured_at_utc", "home_odds", "draw_odds", "away_odds",
]
...
def _validate_no_ai_fields(row: dict[str, Any]) -> None:
    ...
    if present:
        raise ValueError(
            "AI/Structural fields are forbidden by TURKEY_SUPER_LIG_MARKET_ONLY_V1 until a "
            f"separate frozen Turkey Super Lig AI eligibility contract exists: {present}"
        )
...
def validate_capture(row: dict[str, Any]) -> dict[str, Any]:
    ...
    for field in (
        "source_event_id", "source_snapshot_time_utc", "home_team", "away_team",
        "commence_time_utc", "captured_at_utc",
    ):
...
def capture_market_only(row: dict[str, Any], ledger_path: Path = DEFAULT_LEDGER) -> CaptureResult:
    ...
    if same_key:
        ...
        return CaptureResult(
            status="UNCHANGED", league=LEAGUE, protocol=PROTOCOL,
            observation_number=observation_number, target_n=TARGET_N,
            canonical_key=projection["canonical_key"], inserted=False, unchanged=True,
        )
    ...
    return CaptureResult(
        status="INSERTED", league=LEAGUE, protocol=PROTOCOL,
        observation_number=count + 1, target_n=TARGET_N,
        canonical_key=projection["canonical_key"], inserted=True, unchanged=False,
    )


def main() -> int:
    parser = argparse.ArgumentParser(description="Fail-closed Turkey Super Lig MARKET_ONLY prospective capture")
