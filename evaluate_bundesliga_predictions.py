"""Read-only Bundesliga canonical prediction evaluator using Europe/Berlin settlement."""
import pandas as pd
import evaluate_league_predictions as shared
from bundesliga_runtime_config import BUNDESLIGA_RUNTIME_CONFIG
from team_names import normalize_team_name
LEAGUE="BUNDESLIGA"; TIMEZONE="Europe/Berlin"
def settle_bundesliga_predictions(ledger,results):
    ...
    if not (ledger["league"].astype(str)==LEAGUE).all() or not (results["league"].astype(str)==LEAGUE).all(): raise ValueError("Bundesliga evaluator received foreign league")
    ...
    if rv.duplicated(subset=identity,keep=False).any(): raise ValueError("Duplicate Bundesliga result fixture identity")
...
def evaluate_frames(ledger,results):
    settled=settle_bundesliga_predictions(ledger,results); latest=shared.latest_pre_kickoff(settled); return settled,latest,shared.calculate_metrics(settled,view="ALL_SNAPSHOTS"),shared.calculate_metrics(latest,view="LATEST_PRE_KICKOFF_PER_FIXTURE")
def main():
    ledger=shared.load_ledger(LEAGUE); results=shared.load_results(LEAGUE); settled,latest,a,b=evaluate_frames(ledger,results)
    print("BUNDESLIGA CANONICAL PREDICTION EVALUATION"); print("ledger rows:",len(ledger)); print("finished result rows:",len(results)); print("settled rows:",len(settled)); print("MARKET_ONLY rows:",int((ledger.get("prediction_mode",pd.Series(dtype=str))=="MARKET_ONLY").sum())); print("Structural applied rows:",int(ledger.get("structural_applied",pd.Series(dtype=bool)).fillna(False).astype(bool).sum())); shared._print_metrics(a); shared._print_metrics(b); print("evaluation timezone:",TIMEZONE); print("Supabase writes:",False); print("production model used:",False); print("Structural V2 activation:",False)
