...
def prepare_snapshots(
    ...
    return prepare_generic_snapshots(
        snapshots,
        EPL_RUNTIME_CONFIG,
    )
...
def load_upcoming(
    ...
    if not (
        ...
        raise ValueError(
            "Upcoming file contains non-EPL rows"
        )
...
def main() -> None:
    ...
    if (
        ...
        or structural.structural_alpha
        is not None
        or structural.edge_threshold
        is not None
    ):
        raise RuntimeError(
            "Unexpected EPL Structural V2 state"
        )
