"""Support helpers for frozen Primeira Liga nested development V1."""
...
ADMITTED_SEASONS = CONFIGURED_SEASONS[:-1]
...
def _required(source: pd.DataFrame, season: str) -> None:
    ...
    if missing:
        raise ValueError(f"Primeira Liga {season}: missing source columns {sorted(missing)}")
...
def _normalize(source: pd.DataFrame, season: str) -> pd.DataFrame:
    ...
    frame = pd.DataFrame({
        target: source[src] if src in source.columns else pd.NA
    ...
    aliases = PRIMEIRA_LIGA_RUNTIME_CONFIG.aliases
    ...
    if not set(frame["result"].astype(str)).issubset({"H", "D", "A"}):
        raise ValueError(f"Primeira Liga {season}: invalid result code")
    odds = frame[["home_odds", "draw_odds", "away_odds"]]
    if (odds.notna() & (odds <= 1.0)).any().any():
        raise ValueError(f"Primeira Liga {season}: invalid B365 odds")
    return frame


def build_history(work_dir: Path) -> pd.DataFrame:
    config = PRIMEIRA_LIGA_RUNTIME_CONFIG
    if tuple(config.historical_source.season_codes.values()) != CONFIGURED_SEASONS:
        raise RuntimeError("Configured Primeira Liga seasons changed from frozen contract")
    frames = []
    ...
    if observed != set(ADMITTED_SEASONS) or CURRENT_FORBIDDEN_SEASON in observed:
...
def build_trainable_features(history: pd.DataFrame) -> pd.DataFrame:
    ...
    features["warmup_ok"] = (
        (features.home_prior_matches >= minimum)
        & (features.away_prior_matches >= minimum)
    )
